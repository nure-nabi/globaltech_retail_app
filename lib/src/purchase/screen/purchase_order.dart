...
class PurchaseOrderList extends StatefulWidget {
  ...
  @override
  State<PurchaseOrderList> createState() => _PurchaseOrderList();
}

class _PurchaseOrderList extends State<PurchaseOrderList> {
  ...
  Future pickImageFromGallery(PurchaseOrderState state) async {
  ...
  Future<void> pickImageFromCamera(PurchaseOrderState state) async {
  ...
  void initState() {
    ...
    Provider.of<PurchaseOrderState>(context, listen: false).getProductBillWiseOrderList();
    Provider.of<PurchaseOrderState>(context, listen: false).getContext = context;
    final orderState = Provider.of<PurchaseOrderState>(context, listen: false);
    orderState.clear2();
   // Provider.of<SalesTermState>(context, listen: false).termSelected("Bill");
    Provider.of<PurchaseTermState>(context, listen: false).termSelected("Bill");
  ...
  Future<void> _showSaveOrderAlertDialog(BuildContext context, PurchaseOrderState state) async {
    Provider.of<PurchaseOrderState>(context, listen: false).getContext = context;
    return showDialog(
      context: context,
  ...
  Widget build(BuildContext context) {
    ...
    final state = Provider.of<PurchaseOrderState>(context, listen: true);
    // final statePurchaseOrder = context.watch<PurchaseOrderState>();
    //final stateSalesTerm = context.watch<SalesTermState>();
    final statePurchaseTerm =  Provider.of<PurchaseTermState>(context, listen: true);
    ...
    return
      Scaffold(
        appBar: AppBar(title:  Text("Purchase Order List",style: cardTextStyleHeader,),
          actions:  [
            // InkWell(
            //   onTap: () {
            //
            ...
            InkWell(
              onTap :(){
                showDialog(
                  context: context,
                  builder: (BuildContext context) {
                    final orderState = Provider.of<PurchaseOrderState>(context, listen: true);
                   // orderState.clear();
                    return AlertDialog(
                                          ...
                                          child: Row(
                                            children: [
                                              Expanded(
                                                  flex:2,
                                                  child: Container(
                                                    child: Text(statePurchaseTerm.termList[index].pTDesc,style: const TextStyle(fontSize: 14.0),),
                                                  )),
                                                      ...
                                                      onChanged: (value) {
                                                        ...
                                                        if(index == 0)
                                                        {
                                                          ...
                                                          if(statePurchaseTerm.termList[index].sign == "-"){
                                                            state.getBillMapping = statePurchaseTerm.termList[index].pTDesc;
                                                          ...
                                                          state.getBTerm1 = statePurchaseTerm.termList[index].pTCode;
                                                          if(_controllers[index].text == ""){
                                                            _controllersAmount[0].text = "";
                                                            state.setBillDiscountRate = 0.00;
                                                          }
                                                            state.getBSign1 = statePurchaseTerm.termList[index].sign;
                                                          }else if(statePurchaseTerm.termList[index].sign == "+"){
                                                            state.getBillMapping = statePurchaseTerm.termList[index].pTDesc;
                                                          //  Fluttertoast.showToast(msg: statePurchaseTerm.termList[index].pTDesc);
                                                            String disValue = value;
                                                            if(value == ""){
                                                              disValue ="0";
                                                            }
                                                         ...
                                                         if(statePurchaseTerm.termList[index].sign == "-"){
                                                           ...
                                                           double totalAmt = double.parse(state.calculateTotalAmount()) - disAmt;
                                                           double netAmt =  double.parse(state.calculateTotalAmount()) - totalAmt;
                                                           _controllersAmount[1].text = '- ${disAmt.toStringAsFixed(2)}';
                                                           temt = totalAmt;
                                                           state.getBTerm2Rate = double.parse(disValue);
                                                           state.setBillDiscountRateAmount = totalAmt;
                                                           state.getBTerm2Amount =  netAmt;
                                                           state.getBTerm2 = statePurchaseTerm.termList[index].pTCode;
                                                           if(_controllers[index].text == ""){
                                                             _controllersAmount[1].text = "";
                                                             state.setBillDiscountRate = 0.00;
                                                           }
                                                           state.getBSign2 = statePurchaseTerm.termList[index].sign;
                                                         }else  if(statePurchaseTerm.termList[index].sign == "+"){
                                                           String disValue = value;
                                                           if(value == ""){
                                                             disValue ="0";
                                                         ...
                                                         if(statePurchaseTerm.termList[index].sign == "-"){
                                                           ...
                                                           state.getBTerm3 = statePurchaseTerm.termList[index].pTCode;
                                                           if(_controllers[index].text == ""){
                                                             _controllersAmount[2].text = "";
                                                             state.setBillDiscountRate = 0.00;
                                                           }
                                                           state.getBSign3 = statePurchaseTerm.termList[index].sign;
                                                         }else  if(statePurchaseTerm.termList[index].sign == "-"){
                                                           String disValue = value;
                                                           if(value == ""){
                                                             disValue ="0";
            ...
            child: Column(
              ...
              children: [
                ...
                ElevatedButton(
                  ...
                  child:  Text("Confirm Purchase",style:  cardTextStyleHeader),
                ),
              ],
            ),
          ),
        ...
        body: state.allTempOrderList.isNotEmpty
          ...
          itemBuilder: (context, index) {
            ...
            TempPurchaseOrderModel indexData = state.allTempOrderList[index];
...
class EditOrderProductDetails extends StatefulWidget {
  final TempPurchaseOrderModel productDetail;
...
class _EditOrderProductDetailsState extends State<EditOrderProductDetails> {
  ...
  Widget build(BuildContext context) {
    ...
    TempPurchaseOrderModel productDetail = widget.productDetail;
