...
import 'package:retail_app/src/ledger_party_bill_details/api/ledger_party_bill_details_api.dart';
...
class LedgerPartyBillDetailsState extends ChangeNotifier {
  LedgerPartyBillDetailsState();
  ...
  init({required String vNo}) async {
    await clean();
    await getSalesBillReportFromAPI(vNo: vNo);
  ...
  getSalesBillReportFromAPI({required String vNo}) async {
    getLoading = true;
    SalesBillReportModel model = await LedgerPartyBillDetailsApi.apiCall(
