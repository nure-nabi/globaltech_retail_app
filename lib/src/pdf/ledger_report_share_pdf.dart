...
Future<Uint8List> generateLedgerReport({
  List<LedgerReportModel>? salesLedgerReport,
  ...
  final invoice = Invoice(
    salesLedgerReport: salesLedgerReport ?? [],
...
class Invoice {
  Invoice({
    required this.salesLedgerReport,
  ...
  final List<LedgerReportModel> salesLedgerReport;
 ...
 //   Fluttertoast.showToast(msg: dr.length.toString());
  ...
  Widget _contentBalanceFooter(Context context) {
    double drAmt = 0;
    double crAmt = 0;
    for(int i=0; i<salesLedgerReport.length ; i++){
      drAmt += double.parse(salesLedgerReport[i].dr);
      crAmt += double.parse(salesLedgerReport[i].cr);
    }
