...
import 'package:retail_app/src/ledger_report_party_bill/api/ledger_report_party_bill_api.dart';
import 'package:retail_app/src/ledger_report_party_bill/db/ledger_report_party_bill_db.dart';
...
import 'package:retail_app/src/pdf/bill_pdf.dart';
...
class LedgerReportPartyBillState extends ChangeNotifier {
  LedgerReportPartyBillState();
  ...
  init({required String glCode}) async {
    ...
    await getLedgerReportPartyBillApi(glCode: glCode);
  }
  ...
  getLedgerReportPartyBillApi({required String glCode}) async {
    getLoading = true;
    LedgerPartyReportModel model = await LedgerReportPartyBillApi.apiCall(
...
//getDataList = model.data;
  ...
  Future<List<LedgerPartyReportDataModel>>getLedgerReportTotalApi({required String glCode}) async {
    // getLoading = true;
    LedgerPartyReportModel model = await LedgerReportPartyBillApi.apiCall(
      databaseName: _companyDetail.dbName,
      glCode: glCode,
  ...
  onSuccess({required List<LedgerPartyReportDataModel> modelData}) async {
    ...
    for (int i=0; i<modelData.length; i++) {
      ...
      await LedgerReportDatabase.instance.insertData(model);
    }
  ...
  getLedgerWiseFromDB() async {
    await LedgerReportDatabase.instance.getLedgerWiseReport().then((value) {
  ...
  getLedgerDateWiseFromDB( String fromDate1, String toDate1,) async {
    await LedgerReportDatabase.instance.getDateWiseList(fromDate: fromDate, toDate: toDate).then((value) {
  ...
  shareLedger() async {
    ...
    await generateLedgerReport(
      ...
      salesLedgerReport: LedgerWiseList,
      vno: '',
      date: '',
      miti: '',
      source: '',
  ...
  getCashBankPrintFromAPI({required String vno}) async {
    getLoading = true;
    _companyDetail = await GetAllPref.companyDetail();
    CashBankPrintModel reportModel = await LedgerReportPartyBillApi.cashBankPrint(
      databaseName: _companyDetail.dbName,
      vNo: vno,
    );
