...
import 'api/sales_bill_term_api.dart';
import 'db/sales_bill_term_db.dart';
import 'model/sales_bill_term_model.dart';


class SalesTermState extends ChangeNotifier {
  SalesTermState();
  ...
  getDataFromAPI() async {
    SalesBillTermModel salesTermData = await SalesBillTermApi.getTerm(
  ...
  onSuccess({required List<SalesBillTermDataModel> dataModel}) async {
    await SalesTermDatabase.instance.deleteData();

    for (var element in dataModel) {
      await SalesTermDatabase.instance.insertData(element);
    }
    notifyListeners();
  }
  ...
  getTermListFromDB({required String salesTermType}) async {
    await SalesTermDatabase.instance.getTermList(termType: salesTermType).then((value) {
      getTermList = value;
    });
    notifyListeners();
  }
